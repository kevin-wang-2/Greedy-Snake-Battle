...
(function begin() {
    ...
    setInterval(() => {
        ...
        match.execute((result) => {
            ...
            if(result.winner === 1) {
                userData[uidA]["score"] += 0.5 * (Math.max(scoreB - scoreA, 1));
                userData[uidB]["score"] -= 0.2 * (Math.max(scoreB - scoreA, 1));
                userData[uidA]["win"]++;
                userData[uidB]["lose"]++;
            } else if (result.winner === 2) {
                userData[uidA]["score"] -= 0.2 * (Math.max(scoreA - scoreB, 1));
                userData[uidB]["score"] += 0.5 * (Math.max(scoreA - scoreB, 1));
                userData[uidB]["win"]++;
                userData[uidA]["lose"]++;
            } else {
                userData[uidA]["score"] += 0.2 * (scoreB - scoreA);
                userData[uidB]["score"] -= 0.2 * (scoreB - scoreA);
                userData[uidB]["draw"]++;
                userData[uidA]["draw"]++;
            }
            matchCnt--;
